...
export default fp(async (app) => {
  ...
  app.decorate("authenticate", async (request) => {
    ...
    const user = await app.prisma.user.findFirst({ where: { id: token.sub, organisationId: token.organisationId, active: true }, select: { id: true, organisationId: true, role: true } });
    if (!user) throw Object.assign(new Error("Account is inactive or no longer authorised"), { statusCode: 401 });
    request.auth = { userId: user.id, organisationId: user.organisationId, role: user.role };
  });
});
